...
// void setSensitivity(DynamicJsonDocument&);
  ...
  //FixedText_t recordCountdownText(116, 45, Terminal6x8);
...
void  chatGPT_App_Task(void* dApplication){
  ...
  FixedText_t chatGPT_Text(46, 19, Terminal10x17);
  ScrollText_t conn2Intnt(11, 55, 116, ORANGE_RED, 15, 20000, Terminal6x8, 1000);
  thisApp->app_EncoderFn_ptr = volumeControl_Encoder;
  thisApp->app_ButtonFn_ptr = Listen_Process_Button;
  //appMQTT_Parser_Sv->register_MqttServiceFns(selectPattern, selectNumOfBands, setRandomPatterns, setSensitivity);
  appsInitialization(thisApp, micInProcessing_Sv, statusBarClock_Sv, audioOutProcessing_Sv);
  //**************************************************************************************************************************
  humanSpeech = new ScrollText_t (11, 45, 116, CYAN, 25, 20000, Terminal6x8, 0);
  aiResponse = new ScrollText_t(11, 55, 116, YELLOW, 15, 3, Terminal6x8, 0);
  ...
  drawLocalPNG({"/batIcons/openai.png", 12, 12});
  chatGPT_Text.writeColoredString("ChatGPT", WHITE);
  drawLocalPNG({"/batIcons/aiResp.png", 2, 45});
  drawLocalPNG({"/batIcons/hmanSph.png", 2, 55});
  dma_display->drawRect(0, 43, 128, 21, PURPLE_NAVY);   // Draw inner borders2
...
//######################################################################################### */
    ...
    conn2Intnt.scroll_This_Text("Awaiting internet connection...", GREEN_LIZARD);
    while((Applications::internetConnectStatus == false) && (THIS_APP_IS_ACTIVE == pdTRUE)) vTaskDelay(pdMS_TO_TICKS(500));
    conn2Intnt.scroll_Active(STOP_SCROLL);
    humanSpeech->scroll_This_Text("Press the knob and speak.");
    ...
    // printf("In the ChatGPT App Again...\n");
    ...
    humanSpeech->scroll_This_Text(voiceText.c_str());
    // // //*************************************************************************************** */
    heap_caps_free(recordWavFile.psRamFile_P);
    OpenAI_StringResponse result = chat.message(text);
      if(result.length() == 1){
        ...
        mtbAudioPlayer->mtb_Openai_Speech("tts-1", response , "shimmer", "mp3", "1");
        aiResponse->scroll_This_Text(response);
  ...
  conn2Intnt.scroll_Active(STOP_SCROLL);
  aiResponse->scroll_Active(STOP_SCROLL);
  humanSpeech->scroll_Active(STOP_SCROLL);
...
void Listen_Process_Button(button_event_t button_Data){
    ...
    if(Applications::internetConnectStatus == true){
       if(*(Audio_Listening_Sv->serviceT_Handle_ptr) == NULL){
        ...
        drawLocalPNG({"/batIcons/micRec.png", 2, 45});
      } else {
      ...
      statusBarNotif.scroll_This_Text("NO INTERNET CONNECTION. TRY AGAIN LATER.", ORANGE);
      }
      break;
    ...
    case 2:
        chat.clearConversation();
        aiResponse->scroll_This_Text("Conversation cleared.");
        break;
    case 3:
        break;
    default:
...
void micAudioListen_Task(void* d_Service){	// Consider using hardware timer for updating this function to save processor space/time.
  ...
  FixedText_t recordingVoice(11, 45, Terminal6x8);
  uint32_t wavSize;
  int totalSamples = 0;
  recordingVoice.writeColoredString("Listening...", LEMON);
  use_Mic_OR_Dac(I2S_MIC);
  while (THIS_SERV_IS_ACTIVE == pdTRUE && xTimerIsTimerActive(chatPromptTimer_H) == pdTRUE){
  ...
  recordingVoice.writeColoredString("Processing...", GREEN);
  drawLocalPNG({"/batIcons/aiResp.png", 2, 45});
...
void chatPrompt_TimerCallback(TimerHandle_t chatPrompt){
  ...
  if(recordingCountdown --> 0){
    //recordCountdownText.writeColoredString(number.c_str(), LEMON);
    if(recordingCountdown % 2){
      drawLocalPNG({"/batIcons/micRec.png", 2, 45});
    } 
    else {
      drawLocalPNG({"/batIcons/wipe7x7.png", 2,45});
    }
  } else{
    Audio_Listening_Sv->service_is_Running = pdFALSE;
    use_Mic_OR_Dac(DISABLE_I2S_MIC_DAC);
...
uint8_t* create_wav_in_psram(const int16_t* buffer, uint32_t numSamples, uint32_t sampleRate, size_t* wavSize) {
    ...
    if (!wavFile) {
        printf("Failed to allocate PSRAM for WAV file.\n");
        return NULL;
    }
...
//       //printf("WAV file created in PSRAM, size: %d \n", *wavSize);
