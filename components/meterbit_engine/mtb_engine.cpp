...
Mtb_App_Registry_Entry* mtb_App_Registry_Head = nullptr;

static Mtb_App_Registry_Entry* mtb_Find_Registry_Entry(Mtb_User_AppID_t id) {
    for (Mtb_App_Registry_Entry* e = mtb_App_Registry_Head; e; e = e->next) {
        if (e->appID.GenApp == id.GenApp && e->appID.SpeApp == id.SpeApp)
            return e;
    }
    return nullptr;
}
...
Mtb_Applications* mtb_Identify_App_By_ID(Mtb_User_AppID_t dAppID, Mtb_Action_On_App_t action) {
    Mtb_Applications::actionOnApp = action;
    Mtb_App_Registry_Entry* entry = mtb_Find_Registry_Entry(dAppID);
    if (!entry) {
        ESP_LOGE(TAG, "No app registered for ID {%d,%d}", dAppID.GenApp, dAppID.SpeApp);
        return nullptr;
    }
    if (action == LAUNCH_PXP_APP || action == SHOW_PXP_APP_UI)
        return mtb_Launch_This_App(entry->getInstance(), action);
    if (action == KILL_PXP_APP) {
        Mtb_Applications* raw = entry->getRaw();
        return raw ? mtb_Kill_This_App(raw) : nullptr;
    }
    if (action == IDENTIFY_PXP_APP)
        return entry->getRaw();
    return nullptr;
}
