...
EXT_RAM_BSS_ATTR Services *pixAnimClkGif_Sv = new Services(pixAnimClockGif_Task, &pixAnimClockGif_Task_H, "Anim Clk Task", 4096); // THIS APP CANNOT BE IN PSRAM BECAUSE IT WRITES/READS THE FLASH DURING OPERATION.
...
void  pixAnimClock_App_Task(void* dApplication){
  ...
  thisApp->app_ButtonFn_ptr = pixelAnimChangeButton;
  ble_AppCom_Parser_Sv->register_BLE_Com_ServiceFns(setClockTitleAndColor, setPixAnimTheme, setPixAnimClkColors, requestNTP_Time);
  appsInitialization(thisApp, pixAnimClkGif_Sv);
  //************************************************************************************************************************ */
  read_struct_from_nvs("Clock Cols", &clk_Updt, sizeof(Clock_Colors));
  FixedText_t hr_min_Obj(52, 26, Terminal10x17, clk_Updt.hourMinColour);
  FixedText_t sec_Obj(102, 26, Terminal6x8, clk_Updt.secColor);
  FixedText_t am_Pm_Obj(102, 36, Terminal6x8, clk_Updt.meridiemColor);
  FixedText_t wkDay_Obj(45, 50, Terminal6x8, clk_Updt.weekDayColour);
  FixedText_t date_Obj(72, 50, Terminal6x8, clk_Updt.dateColour);

  FixedText_t awaitingNTP_Time_Line1(44, 26, Terminal6x8, LEMON);
  FixedText_t awaitingNTP_Time_Line2(44, 36, Terminal6x8, LEMON);
  FixedText_t awaitingNTP_Time_Line3(44, 46, Terminal6x8, LEMON);
  ...
  headerText = new CentreText_t(63, 9, Terminal10x17, YELLOW);
  headerTextScroll = new ScrollText_t(2, 1, 124, WHITE, 30, 0xFFFF, Terminal10x17, 4000);
  ...
  if(strlen(savedPixAnimClkSet.headerText) < HEADER_TEXT_LIMIT){
    headerText->writeColoredString(savedPixAnimClkSet.headerText, savedPixAnimClkSet.headerTextColor, savedPixAnimClkSet.themeColor[0]);
  } else {
    headerTextScroll->backgroundColor = savedPixAnimClkSet.themeColor[0];
    headerTextScroll->scroll_This_Text(savedPixAnimClkSet.headerText, savedPixAnimClkSet.headerTextColor);
  }
...
//************************************************** */
    ...
    if((now->tm_year) < 124){
    awaitingNTP_Time_Line1.writeString("RETRIEVING");
    awaitingNTP_Time_Line2.writeString("TIME FROM");
    awaitingNTP_Time_Line3.writeString("NETWORK...");
    }
...
//************************************************** */
  ...
  sprintf( rtc_Hr_Min, "%d", pre_Hr );
    ...
    rtc_Am_Pm[1] = 'M';
    rtc_Am_Pm[2] = 0;
    am_Pm_Obj.writeString(rtc_Am_Pm);
  }
...
if(pre_Day != now->tm_mday  || thisApp->elementRefresh){
  ...
  if (pre_Day < 10){
    ...
    rtc_Dated[6] = ' ';
    rtc_Dated[9] = 0;
    wkDay_Obj.writeString(rtc_WkDay);
    date_Obj.writeString(rtc_Dated);
  }
...
void setClockTitleAndColor(JsonDocument& dCommand){
  ...
  if(strlen(title) < HEADER_TEXT_LIMIT){
    headerTextScroll->scroll_Active(STOP_SCROLL);
    headerText->writeColoredString(title, titleColor, savedPixAnimClkSet.themeColor[0]);
  }else{
    headerTextScroll->backgroundColor = savedPixAnimClkSet.themeColor[0];
    headerTextScroll->scroll_Active(STOP_SCROLL);
    headerTextScroll->scroll_This_Text(title, titleColor);
  }

  strcpy(savedPixAnimClkSet.headerText, title);
  savedPixAnimClkSet.headerTextColor = titleColor;
  write_struct_to_nvs("pixAnimClk", &savedPixAnimClkSet, sizeof(PixAnimClkSettings_t));
...
void setPixAnimClkColors(JsonDocument& dCommand){
  ...
  read_struct_from_nvs("Clock Cols", &clk_Cols, sizeof(Clock_Colors));
...
//   else printf("Left Hand Direction Selected");
...
void pixAnimClockGif_Task(void* d_Service){	// Consider using hardware timer for updating this function to save processor space/time.
  ...
  while (THIS_SERV_IS_ACTIVE == pdTRUE){
		...
		while (THIS_SERV_IS_ACTIVE == pdTRUE && (entry = readdir(dir)) != NULL){
			char fullPath[300];
		...
		if (width < 65 && height < 33){
			...
			for (uint32_t show_Duration = 0; THIS_SERV_IS_ACTIVE == pdTRUE && show_Duration < set_Duration; ){
				while (THIS_SERV_IS_ACTIVE == pdTRUE && gd_get_frame(gif)){
