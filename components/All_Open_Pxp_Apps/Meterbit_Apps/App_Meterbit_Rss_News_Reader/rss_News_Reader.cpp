...
EXT_RAM_BSS_ATTR Mtb_Applications_StatusBar *rssNewsApp;
Mtb_Applications* rssNewsApp_GetInstance() {
    if (!rssNewsApp) rssNewsApp = new Mtb_Applications_StatusBar(rssNewsApp_Task, &rssNewsApp_Task_H, "RSS News", {1,1}, 4096);
    return rssNewsApp;
}
MTB_REGISTER_APP(rssNewsApp, 1, 1)
...
void rssNewsApp_Task(void* dApplication) {
  ...
  THIS_APP->mtb_App_Set_Ble_Comm_Fns(updateSourceSelection);
