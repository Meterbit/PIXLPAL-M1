...
//**************************************************************************************
Mtb_ScrollText_t::Mtb_ScrollText_t(uint16_t x, uint16_t y, uint16_t w, const uint8_t * f, uint16_t c, uint16_t s, uint16_t p, uint64_t scr_hld, uint8_t makeBeep){
    xPos = x;
    yPos = y;
    width = w;
    color = c;
...
void  mtb_Text_Scrolls_Init(void){
    ...
    mtb_Scroll_Tasks_Sv[0]->service = mtb_Scroll_Text_0_Task;
    mtb_Scroll_Tasks_Sv[1]->service = scrollText_1_Task;
    mtb_Scroll_Tasks_Sv[2]->service = scrollText_2_Task;
    mtb_Scroll_Tasks_Sv[3]->service = scrollText_3_Task;
    mtb_Scroll_Tasks_Sv[4]->service = scrollText_4_Task;
}
//**************************************************************************************
void Mtb_ScrollText_t::mtb_Scroll_This_Text(const char* dText){
    ...
    if(this == (&statusBarNotif)){
        ...
        mtb_Launch_This_Service(mtb_Scroll_Tasks_Sv[0]);
    }else {
        for (uint8_t i = 1; i < 5; i++){
        if((scrollText_Handles[i]) == NULL){
            this->scrollTaskHandling = i;
            xQueueSend(scroll_Q[i], this, portMAX_DELAY);
            mtb_Launch_This_Service(mtb_Scroll_Tasks_Sv[i]);
            break;
            }
        }
    }
            ...
            if(*this == holder){
                ...
                while (xQueuePeek(scroll_Q[i], &holder, pdMS_TO_TICKS(10)) == pdTRUE){
                   ...
                   delay(1);
                    ...
                    mtb_Panel_Fill_Rect(holder.xPos, holder.yPos, holder.xPos + holder.width - 1, holder.yPos + holder.height - 1, holder.backgroundColor);
                }
                return 1;
                }
            }
...
//                    delay(1);
//                 } 
//                 mtb_Panel_Fill_Rect(holder.xPos, holder.yPos, holder.width, holder.height, holder.backgroundColor);
