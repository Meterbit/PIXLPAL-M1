...
void bleSettings(DynamicJsonDocument& dCommand){
...
//**01*********************************************************************************************************************
void current_BleDevice(const char* pxpBleName){
char ble_Contd[150] = "{\"pxp_command\": 1, \"connected\": 1, \"pxpBleName\": \"";
String ble_Nt_Contd = "{\"pxp_command\": 1, \"connected\": 0}";

if (pxpBleName == NULL) bleSettingsComSend(bleSettingsRoute, ble_Nt_Contd);
else if(strlen(pxpBleName) > 0){
  strcat(ble_Contd, pxpBleName);
  strcat(ble_Contd, "\"}");
  bleSettingsComSend(bleSettingsRoute, String(ble_Contd));
}
}

//**02*********************************************************************************************************************
void set_PxpBleName(DynamicJsonDocument& dCommand){ 
